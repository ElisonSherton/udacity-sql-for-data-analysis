...
-- Try using LIMIT yourself below by writing a query that displays all the 
-- data in the occurred_at, account_id, and channel columns of the 
-- web_events table, and limits the output to only the first 15 rows.

SELECT occurred_at, account_id, channl 
FROM web_events 
LIMIT 15;
...
-- Write a query that displays the order ID, account ID, and total dollar amount for all the orders, sorted 
-- first by the account ID (in ascending order), and then by the total dollar amount (in descending order).
...
-- Write a query that Pulls the first 5 rows and all columns from the orders table that have a dollar
...
SELECT id, account_id,
       (poster_amt_usd / (poster_amt_usd + gloss_amt_usd + standard_amt_usd)) AS pct_poster
FROM orders

-- Find from accounts table All the companies whose names start with 'C'.
...
-- Find from accounts table All companies whose names contain the string 'one' somewhere in the name.
...
-- Find from accounts table All companies whose names end with 's'.
